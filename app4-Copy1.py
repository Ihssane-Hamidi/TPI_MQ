...
def calc_volatilite_brent(prices, tickers, rallies):
    """Calcule la volatilité annualisée moyenne pendant les périodes de rallies."""
    results = {}
    all_rally_dates = []
    for start, end in rallies:
        all_rally_dates.extend(pd.date_range(start, end))
    
    for t in tickers:
        if t not in prices.columns: continue
        px = prices[t].dropna()
        target_dates = px.index.intersection(pd.DatetimeIndex(all_rally_dates))
        if len(target_dates) < 10: continue
        
        # Volatilité annualisée des rendements journaliers
        vol = px.loc[target_dates].pct_change().std() * np.sqrt(252)
        results[t] = vol
    return results

def page_brent(valid, prices, rallies, score_col, score_label,
               secteur_col, quintile_col, total_panel, dataset_label):
    """Page Analyse Brent — comparaison modèle général vs pendant hausse Brent (Rendement ou Volatilité)."""
    ...
    with c2:
        # NOUVEAU : Choix de la métrique
        metric_choice = st.selectbox(
            "Métrique d'analyse",
            ["Rendement", "Volatilité"],
            key="brent_metric_choice"
        )
    with c3:
    ...
    # Définition de la variable dépendante générale
    # Note: Si votre dataframe 'valid' n'a que des colonnes 'Rendement_...', 
    # assurez-vous d'avoir aussi 'Volatilite_...' si vous voulez comparer la vol générale.
    # Ici on adapte le nom de la colonne cible :
    prefix = "Rendement" if metric_choice == "Rendement" else "Volatilite"
    dep_gen = f"{prefix}_{period_gen}"
    
    # Sécurité si la colonne volatilité générale n'existe pas encore dans valid
    if dep_gen not in valid.columns and metric_choice == "Volatilité":
        st.warning(f"La colonne {dep_gen} n'est pas disponible. Comparaison impossible avec le mode général.")
        return

    # --- CALCUL PENDANT HAUSSE BRENT ---
    with st.spinner(f"Calcul de la {metric_choice} pendant les hausses Brent..."):
        tickers_valid = valid["ticker"].dropna().tolist()
        if metric_choice == "Rendement":
            data_brent = calc_rendement_brent(prices, tickers_valid, rallies)
            target_col_brent = "Rendement_Brent"
            fmt = ".1%"
        else:
            data_brent = calc_volatilite_brent(prices, tickers_valid, rallies)
            target_col_brent = "Volatilite_Brent"
            fmt = ".2f"

    valid2 = valid.copy()
    valid2[target_col_brent] = valid2["ticker"].map(data_brent)
    n_brent = valid2[target_col_brent].notna().sum()

    st.markdown(f"""<div class="note-box">
        <b>{metric_choice}</b> calculée sur {len(rallies)} période(s) de hausse Brent.<br>
        <b>{n_brent}</b> entreprises analysées.<br>
    ...
    def run_ols(df_in, dep_var):
        if dep_var not in df_in.columns: return None, 0
        df_r = df_in[[c for c in base_cols + [dep_var] if c in df_in.columns]].dropna().copy()
        if df_r.empty: return None, 0
    ...
    model_gen, n_gen = run_ols(valid2, dep_gen)
    model_brent, n_brent2 = run_ols(valid2, target_col_brent)

    if not model_gen or not model_brent:
        st.error("Données insuffisantes pour faire tourner les régressions.")
        return

    # --- COMPARAISON DES COEFFICIENTS ---
    coef_gen = model_gen.params.get("Score_principal", 0)
    pval_gen = model_gen.pvalues.get("Score_principal", 1)
    coef_br  = model_brent.params.get("Score_principal", 0)
    pval_br  = model_brent.pvalues.get("Score_principal", 1)
    delta    = coef_br - coef_gen

    col1, col2, col3, col4 = st.columns(4)
    metric_card(col1, f"Coef. {metric_choice} - Général", f"{coef_gen:+.3f} {sig_stars(pval_gen)}", pval_gen < 0.05)
    metric_card(col2, f"Coef. {metric_choice} - Brent-up", f"{coef_br:+.3f} {sig_stars(pval_br)}", pval_br < 0.05)
    metric_card(col3, "Δ Coefficient", f"{delta:+.3f}", delta > 0 if metric_choice == "Rendement" else delta < 0)
    metric_card(col4, "R² (Gén / Brent)", f"{model_gen.rsquared_adj:.2f} / {model_brent.rsquared_adj:.2f}", True)

    # --- GRAPHIQUE PAR SECTEUR ---
    st.markdown(f'<p class="section-title">Effet du score sur la {metric_choice} par secteur</p>', unsafe_allow_html=True)
    
    def get_sector_effects(df_in, dep_var):
        df_r = df_in[[c for c in [score_col, secteur_col, dep_var] if c in df_in.columns]].dropna().copy()
        df_r = df_r.rename(columns={score_col: "Score_principal"})
        m = smf.ols(f"{dep_var} ~ Score_principal * C({secteur_col})", data=df_r).fit(cov_type="HC3")
        
        secteurs = sorted(df_in[secteur_col].dropna().unique().tolist())
        ref = secteurs[0]
        effects = {}
        for s in secteurs:
            base = m.params.get("Score_principal", 0)
            if s == ref: effects[s] = base
            else:
                k = f"Score_principal:C({secteur_col})[T.{s}]"
                effects[s] = base + m.params.get(k, 0)
        return effects

    try:
        eff_gen = get_sector_effects(valid2, dep_gen)
        eff_br = get_sector_effects(valid2, target_col_brent)
        
        df_plot = pd.DataFrame({
            "Secteur": list(eff_gen.keys()),
            "Général": list(eff_gen.values()),
            "Brent-up": [eff_br.get(s, np.nan) for s in eff_gen.keys()]
        }).sort_values("Général")

        fig = go.Figure()
        fig.add_trace(go.Bar(name="Général", x=df_plot["Secteur"], y=df_plot["Général"], marker_color="rgba(99,179,237,0.8)"))
        fig.add_trace(go.Bar(name="Brent-up", x=df_plot["Secteur"], y=df_plot["Brent-up"], marker_color="rgba(251,191,36,0.8)"))
        fig.update_layout(**PLOTLY_LAYOUT, barmode="group", height=350)
        st.plotly_chart(fig, use_container_width=True)
    except:
        st.info("Calcul des interactions sectorielles indisponible pour cette sélection.")

    # --- COMPARAISON PAR QUINTILE ---
    st.markdown(f'<p class="section-title">Moyenne par quintile ({metric_choice})</p>', unsafe_allow_html=True)
    ...
    for q in qs:
        ...
        if not sub.empty:
            m_gen = sub[dep_gen].mean() if dep_gen in sub else 0
            m_br = sub[target_col_brent].mean()
            q_data.append({"Quintile": q, "Général": m_gen, "Brent-up": m_br})
    
    df_q = pd.DataFrame(q_data)
    fig_q = go.Figure()
    fig_q.add_trace(go.Bar(name="Général", x=df_q["Quintile"], y=df_q["Général"], marker_color="gray", opacity=0.5))
    fig_q.add_trace(go.Bar(name="Brent-up", x=df_q["Quintile"], y=df_q["Brent-up"], 
                          marker_color=[QUINTILE_COLORS.get(q, "#white") for q in df_q["Quintile"]]))
    fig_q.update_layout(**PLOTLY_LAYOUT, barmode="group", height=300, yaxis=dict(tickformat=fmt))
    st.plotly_chart(fig_q, use_container_width=True)

    with st.expander("Détails des régressions"):
        st.text("--- MODÈLE GÉNÉRAL ---")
        st.text(model_gen.summary().as_text())
        st.text("--- MODÈLE BRENT-UP ---")
