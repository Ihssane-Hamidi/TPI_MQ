...
def calc_metriques_brent(prices, tickers, rallies):
    ...
    for t in tickers:
        ...
        if n_days >= 10:
            ...
            vol_records[t] = float(np.std(all_rets) * np.sqrt(252))
        else:
            rdt_records[t] = np.nan
            vol_records[t] = np.nan
...
def page_brent(valid, prices, rallies, score_col, score_label,
    ...
    with c2:
        dep_choice_b = st.selectbox(
            "Variable dépendante",
            ["Rendement", "Volatilite"],
            format_func=lambda x: {"Rendement": "Rendement", "Volatilite": "Volatilité"}[x],
            key="brent_dep",
        )
    with c3:
    ...
    # ── Calcul rendement + volatilité pendant hausse Brent ───────────────
    with st.spinner("Calcul des métriques pendant les hausses Brent..."):
    ...
    model_brent, n_brent2 = run_ols(valid2, dep_brent, "Pendant hausse Brent")
    ...
    m_brent_int = run_ols_interaction(valid2, dep_brent)
    ...
    for q in ["Q1", "Q2", "Q3", "Q4", "Q5"]:
        ...
        rows_q.append({
            "Quintile": q,
            "N": len(sub),
            f"{dep_choice_b} {PERIODS_LABELS[period_gen]} (moy.)": f"{sub[dep_gen].mean():.1%}" if dep_gen in sub.columns else "N/A",
            f"{dep_choice_b} Brent-up (moy.)": f"{sub[dep_brent].mean():.1%}" if dep_brent in sub.columns else "N/A",
            "Δ (Brent − Général)": f"{(sub[dep_brent].mean() - sub[dep_gen].mean()):+.1%}" if (dep_gen in sub.columns and dep_brent in sub.columns) else "N/A",
        })
    ...
    if rows_q:
        ...
        fig_q.add_trace(go.Bar(
            name=f"Général ({PERIODS_LABELS[period_gen]})",
            x=df_q["Quintile"],
            y=[float(v.replace("%","").strip())/100 if v != "N/A" else 0 for v in df_q[f"{dep_choice_b} {PERIODS_LABELS[period_gen]} (moy.)"]],
        ...
        fig_q.add_trace(go.Bar(
            name="Pendant hausse Brent",
            x=df_q["Quintile"],
            y=[float(v.replace("%","").strip())/100 if v != "N/A" else 0 for v in df_q[f"{dep_choice_b} Brent-up (moy.)"]],
            marker_color=[QUINTILE_COLORS[q] for q in df_q["Quintile"]],
